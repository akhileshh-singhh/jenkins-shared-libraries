def call(Map config = [:]) {
    def subfolderName = config.subfolderName ?: '.' // Adjust if your repo root contains a subfolder
    ...
    pipeline {
        ...
        stages {
            stage('Checkout Code') {
                steps {
                    // Jenkins checks out natively in its workspace root cleanly
                    checkout([
                        $class: 'GitSCM',
            ...
            stage('Setup Environment & Dependencies') {
                steps {
                    script {
                        // If your project files are in the root of the workspace, leave subfolderName as '.'
                        // If they are nested, specify the folder name here
